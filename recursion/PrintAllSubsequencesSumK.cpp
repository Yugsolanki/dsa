...
// Print all subsequences that sum upto k
void printSubsequencesSumK(int i, vector<int> &temp, int sum, vector<int> &arr, int k)
{
    ...
    printSubsequencesSumK(i + 1, temp, sum + arr[i], arr, k);

    // Case where element is excluded
    temp.pop_back();
    printSubsequencesSumK(i + 1, temp, sum, arr, k);
}
